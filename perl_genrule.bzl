...
def generate_commands(binary, assembly_flavor, srcs_to_outs, srcs_to_outs_dupes, ctx):
    ...
    for src, out in srcs_to_outs_dupes.items():
        ...
        src_files = src_files + intermediate_src_files
    if ctx.attr.is_unix:
        return ",".join([ctx.expand_make_variables(command, "$(PERL)", {"PERL": ctx.toolchains["@rules_perl//perl:toolchain_type"].perl_runtime.interpreter}) for command in commands]), src_files, out_files
    else:
